      ...
      <header className="sticky top-0 z-40 border-b border-gray-800 bg-gray-950/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1.5 sm:gap-2">
            ...
            <button onClick={() => setShowCompiler(true)} className={tabCls("practice")}>Practice</button>
        ...
        {activeTab === "home" && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid gap-5">
              {PRODUCTS.map((p) => (
                <ProductCard key={p.id} product={p} copied={copied} onBuy={setSelectedProduct} onJoinGroup={handleJoinGroup} onCopyForm={handleCopyForm} />
              ))}
            </div>
          </div>
        )}
