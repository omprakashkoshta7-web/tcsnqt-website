...
export default function ProductCard({ product, onBuy, onJoinGroup, onCopyForm, onViewPdf, copied, isPurchased }) {
  ...
  return (
    <div className={`group relative bg-gray-900/70 border border-gray-800 border-l-4 ${cs.border} rounded-xl p-4 sm:p-6 transition-all duration-300 hover:border-l-[6px] hover:bg-gray-900/90 hover:shadow-lg hover:shadow-blue-900/20`}>
      <div className="flex justify-between flex-wrap items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isPurchased && (
              <span className="px-2 py-0.5 bg-emerald-500/15 text-emerald-300 text-[10px] font-bold uppercase rounded-full border border-emerald-500/30">
                ✓ Purchased
              </span>
            )}
          </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {isPurchased ? (
          <button onClick={() => onViewPdf(product)} className="inline-flex items-center gap-1.5 px-3 sm:px-4 py-2 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-400 hover:to-green-500 text-white text-xs sm:text-sm font-bold rounded-lg transition-all shadow-lg shadow-emerald-900/40 animate-pulse-slow">
